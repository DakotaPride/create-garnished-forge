...
public class CreateGarnishedMobConditions extends MobConditions {
    ...
    public static void applyConditions(LivingEntity entity, LivingEntity attacker, DamageSource source) {
        ...
        createDropConditions(entity, EntityType.SHULKER, attacker, Items.SHULKER_SHELL, config.shulkerShellMaxCount.get(), config.chanceToDropExtraShulkerShells.get(), source, config.disableShulkerDrops.get());

        createDropConditions(entity, EntityType.PHANTOM, attacker, Items.PHANTOM_MEMBRANE, config.phantomMembraneMaxCount.get(), config.chanceToDropExtraPhantomMembranes.get(), source, config.disablePhantomDrops.get());

        createDropConditions(entity, EntityType.WITCH, attacker, Items.REDSTONE, config.redstoneDustMaxCount.get(), config.chanceToDropExtraRedstoneDust.get(), source, config.disableWitchDrops.get());
        createDropConditions(entity, EntityType.WITCH, attacker, Items.GLOWSTONE_DUST, config.glowstoneDustMaxCount.get(), config.chanceToDropExtraGlowstoneDust.get(), source, config.disableWitchDrops.get());
        createDropConditions(entity, EntityType.WITCH, attacker, Items.NETHER_WART, config.netherWartMaxCount.get(), config.chanceToDropNetherWart.get(), source, config.disableWitchDrops.get());

        createDropConditions(entity, EntityType.RAVAGER, attacker, AllItems.ZINC_NUGGET.get(), config.zincNuggetMaxCount.get(), config.chanceToDropZincNugget.get(), source, config.disableRavagerDrops.get());

        createDropConditions(entity, EntityType.SNIFFER, attacker, CreateGarnishedItems.SNIFFER_FLUFF.get(), config.snifferFluffMaxCount.get(), config.chanceToDropSnifferFluff.get(), source, config.disableSnifferDrops.get());

        createDropConditions(entity, EntityType.CREEPER, attacker, Items.GUNPOWDER, config.gunpowderMaxCount.get(), config.chanceToDropGunpowder.get(), source, config.disableCreeperDrops.get());

        createDropConditions(entity, EntityType.GHAST, attacker, Items.GUNPOWDER, config.gunpowderMaxCount.get(), config.chanceToDropGunpowder.get(), source, config.disableGhastDrops.get());
        createDropConditions(entity, EntityType.GHAST, attacker, Items.GHAST_TEAR, config.extraGhastTearMaxCount.get(), config.chanceToDropExtraGhastTear.get(), source, config.disableGhastDrops.get());

        createDropConditions(entity, EntityType.BREEZE, attacker, Items.BREEZE_ROD, config.extraBreezeRodMaxCount.get(), config.chanceToDropExtraBreezeRod.get(), source, config.disableBreezeDrops.get());
        createDropConditions(entity, EntityType.BLAZE, attacker, Items.BLAZE_ROD, config.extraBlazeRodMaxCount.get(), config.chanceToDropExtraBlazeRod.get(), source, config.disableBlazeDrops.get());

        createDropConditions(entity, EntityType.ENDERMAN, attacker, Items.ENDER_PEARL, config.extraEnderPearlMaxCount.get(), config.chanceToDropExtraEnderPearl.get(), source, config.disableEndermanDrops.get());
    }
    ...
    private static void registerGelatinousCreatureDropConditions(LivingEntity entity, LivingEntity attacker, DamageSource source) {
        if (!config.disableGelatinousCreatureDrops.get()) {
            createDropConditions(entity, EntityType.SLIME, attacker, CreateGarnishedItems.ELASTIC_STRAND.get(), config.elasticStrandMaxCount.get(),
                    config.chanceToDropElasticStrand.get(), source, config.disableGelatinousCreatureDrops.get());
            createDropConditions(entity, EntityType.SLIME, attacker, Items.SLIME_BALL, config.slimeBallMaxCount.get(),
                    config.chanceToDropExtraSlimeBalls.get(), source, config.disableSlimeDrops.get());
    ...
    private static void registerSpiderDropConditions(LivingEntity entity, LivingEntity attacker, DamageSource source) {
        if (!config.disableAllSpiderDrops.get()) {
            if (!config.disableGlobalHatchetDrops.get()) {
