...
public class SpecialEffectsBlock {
    ...
    public static class Abyssal {
        public static void getParticles(BlockState state, Level level, BlockPos pos, Random random) {
            for(int i = 0; i < 4; ++i) {
    ...
    public static class Carnotite {
        ...
        public static void getPlacement(UseOnContext ctx) {
            ...
            if (state.is(GarnishedTags.CARNOTITE_BLOCKS)) {
                level.playLocalSound(pos.getX(), pos.getY(), pos.getZ(), SoundEvents.STONE_BREAK, SoundSource.BLOCKS, 1.0F, 1.0F, true);
                stack.shrink(1);
    ...
    public static class Unstable {
        ...
        public static void getParticles(BlockPos pos, Random random, Level level, BlockState state, ParticleOptions options) {
            for(int i = 0; i < 4; ++i) {
                double d0 = (double) pos.getX() + random.nextDouble();
                double d1 = (double) pos.getY() + random.nextDouble();
                double d2 = (double) pos.getZ() + random.nextDouble();
