...
public class GarnishedBlocks {
	...
	public static final BlockEntry<NetherLichenBlock> SORROWFUL_LICHEN =
			REGISTRATE.block("sorrowful_lichen", NetherLichenBlock::new)
					.blockstate((ctx, pov) -> pov.simpleBlock(ctx.get(), AssetLookup.standardModel(ctx, pov)))
					.simpleItem()
					.properties(p -> p.lightLevel(NetherLichenBlock.emission(4)).color(MaterialColor.GLOW_LICHEN)
							.noCollission().strength(0.2F).sound(SoundType.GLOW_LICHEN)).register();
	...
	public static final BlockEntry<PottedSoulRootsBlock> POTTED_SOUL_ROOTS =
			REGISTRATE.block("potted_soul_roots", PottedSoulRootsBlock::new).initialProperties(() -> Blocks.POTTED_CRIMSON_FUNGUS).register();
