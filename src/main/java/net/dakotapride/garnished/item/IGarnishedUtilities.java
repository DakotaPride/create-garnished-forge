...
public interface IGarnishedUtilities {
	...
	default void addEffectTooltip(List<Component> tooltip, MobEffect effect, int amplifier, float duration) {
		tooltip.add(Component.translatable("text.garnished.applies_effect", Component.translatable(effect.getDescriptionId()), formatAmplifier(amplifier), formatDuration(new MobEffectInstance(effect), duration, 1)).withStyle(effect.getCategory().getTooltipFormatting()));
	}
	...
	default void triggerConditionalEffect(int value, float chance, LivingEntity entity) {
		...
		boolean hasSugarHigh = entity.hasEffect(GarnishedEffects.SUGAR_HIGH.get());
		// Freezing functionality
		boolean hasFreezing = entity.hasEffect(GarnishedEffects.FREEZING.get()) || entity.isFreezing();
		...
		boolean hasOmen = entity.hasEffect(MobEffects.BAD_OMEN);
		// Fire functionality
		...
		if (!level.isClientSide) {
			...
			if (value == 2) {
				has = hasHunger;
				if (has && floatChance <= chance)
					entity.addEffect(new MobEffectInstance(GarnishedEffects.THORNS.get(), tick * 24, 1));
			}
			if (value == 3) {
				has = hasLevitation;
				if (has && floatChance <= chance)
